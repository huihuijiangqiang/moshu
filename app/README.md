...
| `/deconstruct` | `DeconstructView` | P1 参考作品拆书分析 |
...
真实登录与会话轮换、项目/工作室权限、行内与整章生成、额度结算、版本历史、全书校订、拆书分析、
一致性守卫和导出备份均已接通后端。当前仍保留的产品边界：

- 大纲支持按钮换序，也支持卷和章节的原生拖拽排序；跨卷拖动章节会追加到目标卷末尾
- 工作室已有成员、角色、作品共享、章节任务流和成员产量看板，不包含 Yjs/Hocuspocus 同屏实时编辑
- 已有套餐和积分账本，不包含支付渠道、订阅购买与发票流程
- 关系图、地图、出版排版和第三方平台发布属于后续增强
