...
- ✅ 升级及候选审阅实现后 151 个前端测试、TypeScript 类型检查和 Vite 生产构建全部通过
...
2. ✅ 首版时间区间影响集裁剪：只对归属与地点两类具备严格重叠语义的规则裁掉确定不相交候选；
   归属（`owns`）的可证明条件已下推到 SQL，地点（`located_at`）仍在内存最终裁剪；
   生死、知情、能力、无时间和模糊时间保持保守召回，旧/新版本区间共同参与边界计算
...
**当前优化**：影响键查询先用保守的 SQL 条件过滤确定不相交的 `owns` 候选，随后以内存裁剪作为最终安全门；
`interval_filter_applied` 会在任务结果中标记本次是否启用该过滤。`located_at`、未知/畸形区间以及无法证明安全的
谓词继续保留候选，避免漏检。

**后续优化**：基于有授权真实长篇分布和 PostgreSQL `EXPLAIN` 结果，评估 `located_at` 下推和 500 键降级阈值；
...
### 3. 增量影响集的数据库下推仍待优化
**描述**：实体、谓词族、旧版事实和归属/地点区间的安全影响裁剪已实现；`owns` 的可证明不相交条件
已在 PostgreSQL 查询阶段下推，`located_at` 仍在内存最终裁剪。高频核心人物在 PostgreSQL 中仍可能先读取较大的地点候选集。

**风险**：规则 CPU 已明显减少，归属候选的数据库成本已下降，但地点候选仍承担被裁掉候选的传输与 ORM 对象构造成本。
...
3. 模糊时间已完成规范化区间、作者确认、项目级 reflow 与依赖复检；归属/地点影响集已做保守区间裁剪，数据库查询下推仍未完成；仲裁只提供建议，不自动处置
