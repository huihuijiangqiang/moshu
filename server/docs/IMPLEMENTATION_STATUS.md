...
- ✅ 1073 个单元/功能测试通过（SQLite in-memory，mock embedding/LLM）
- ✅ 前端 90 个测试、TypeScript 类型检查和生产构建通过
...
#### 单元测试（1073 passed，SQLite in-memory，mock providers）

**全量测试结果**：1073 passed, 37 skipped（未设置集成测试 URL 时）, 0 warnings；前端 90 passed
...
- ⚠️ 模糊区间会保存并展示，但作者确认区间的交互尚未完成
- ⚠️ 项目 reflow 只复检已有结构化 claim，不重新调用抽取模型

**后续需要**：
1. 模糊时间区间的作者确认与覆盖值
2. 多剧情线时间板和日历视图
3. 基于实际长篇分布的时间区间影响集裁剪
...
### 测试（1073 passed；另有 36 个真实 PostgreSQL 测试通过）
- `server/tests/` - 单元/功能测试（1073 passed）
- `app/src/**/*.spec.ts` - 前端测试（90 passed）
