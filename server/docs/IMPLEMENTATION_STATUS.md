...
- ✅ 1206 个单元/功能测试通过（SQLite in-memory，mock embedding/LLM）
...
#### 单元测试（1206 passed，SQLite in-memory，mock providers）

**全量测试结果**：1206 passed, 37 skipped（未设置集成测试 URL 时）；前端 151 passed
...
### 测试（1206 passed；另有 37 个真实 PostgreSQL 测试通过）
- `server/tests/` - 单元/功能测试（1206 passed）
...
墨枢一致性后端已完成核心数据模型、服务层、API 端点和异步任务定义，1206 个单元/功能
